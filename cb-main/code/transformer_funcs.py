...
def Attention_Matrix(K, Q, use_mask=False):
    ...
    score = tf.linalg.matmul(Q, tf.linalg.matrix_transpose(K))
    
    score = score / np.sqrt(window_size_keys)
...
class Atten_Head(tf.keras.layers.Layer):
    def __init__(self, input_size, output_size, use_mask):        
        ...
        self.Kw = self.add_weight("kw", shape=(input_size, output_size), initializer='random_normal', trainable=True)
        self.Vw = self.add_weight("vw", shape=(input_size, output_size), initializer='random_normal', trainable=True)
        self.Qw = self.add_weight("qw", shape=(input_size, output_size), initializer='random_normal', trainable=True)
    ...
    def call(self, inputs_for_keys, inputs_for_values, inputs_for_queries):
        ...
        K = tf.linalg.matmul(inputs_for_keys, self.Kw)
        V = tf.linalg.matmul(inputs_for_values, self.Vw)
        Q = tf.linalg.matmul(inputs_for_queries, self.Qw)
