...
START_TOKEN = "*START*"
UNK_TOKEN = "*UNK*"
FRENCH_WINDOW_SIZE = 15
ENGLISH_WINDOW_SIZE = 15
##########DO NOT CHANGE#####################
...
def convert_to_string(vocab, ids):
    string = []
    
    for i in ids[0]:
        string.append(vocab[i])
...
def get_data(french_training_file, english_training_file, french_test_file, english_test_file):
    ...
    ftrainf = read_data(french_training_file)
    etrainf = read_data(english_training_file)
    ftestf = read_data(french_test_file)
    etestf = read_data(english_test_file)

    #2) Pad training data (see pad_corpus)
    
    ftrainf, etrainf = pad_corpus(ftrainf, etrainf)

    #3) Pad testing data (see pad_corpus)
    
    ftestf, etestf = pad_corpus(ftestf, etestf)

    #5) Build vocab for english (see build_vocab)
    
    evocab, etoken = build_vocab(etrainf)
    
    print(np.array(etrainf).shape)

    #6) Convert training and testing english sentences to list of IDS (see convert_to_id)
    
    etrainf = convert_to_id(evocab, etrainf)
    etestf = convert_to_id(evocab, etestf)
 
    #7) Convert training and testing french sentences to list of IDS (see convert_to_id)
    
    ftrainf = convert_to_id(evocab, ftrainf)
    ftestf = convert_to_id(evocab, ftestf)
    

    return etrainf, etestf, ftrainf, ftestf, evocab, etoken
